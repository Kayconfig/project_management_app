...
function getColumnFromBackend(backlog = [], todo = [], done = []) {
  return {
    backlog: {
      name: "Backlog",
      items: backlog,
...
function Kanban({ updateTask }) {
  const { projectId, projectname } = useParams();
  const [columns, setColumns] = useState(getColumnFromBackend());
  const { token } = useContext(authContext);
  const [todos, setTodos] = useState([]);
  const [backlogs, setBackLogs] = useState([]);
  const [dones, setDones] = useState([]);
  useEffect(() => {
    setColumns(getColumnFromBackend(backlogs, todos, dones));
  }, [todos, backlogs, dones]);
  const onDragEnd = (result, columns, setColumns) => {
    ...
    if (source.droppableId !== destination.droppableId) {
      ...
      axios
        .request({
          url: `https://kojjac.herokuapp.com/tasks/update/${removed._id}`,
          method: "post",
          headers: { token: token },
  ...
  useEffect(() => {
    axios
      .request({
        url: `${process.env.REACT_APP_BACKEND_URL}/tasks/getTasks/${projectId}/backlog`,
        headers: { token: token },
        method: "get",
      })
      .then((response) => {
